...
if testing_local:
    ...
    output_dir = '.\\CycleGAN-custom\\output_test_no_cycle\\'
    train_epochs = 1
    ...
    data_dir = './maps/'
    output_dir = './output_train_0_no_cycle/'
    train_epochs = 25
...
if __name__ == '__main__':
    ...
    for epoch in range(train_epochs): 
        ...
        for i, data in enumerate(dataset): # loops over A and B images
            ...
            # optimize A and B generators: UPDATE: NO CYCLE LOSS CALCULATED
            ...
            # optimize A and B discriminators
            ...
            # get and write losses -> [G_A, G_B, D_A, D_B] <- note this order
            if iters % save_loss_freq_iters == 0:
                losses = [iters, float(G_ls[0]), float(G_ls[1]), float(D_A_loss), float(D_B_loss)]
                losses = [format(num, ".4f") for num in losses]

                file_loc = os.path.join(output_dir, 'losses_no_cycle.txt')
                ...
                start_time = time.time()
                print('Iter: ',iters, f" in {elapsed_time:.2f} s, "," Losses_no_cycle: ",losses)
